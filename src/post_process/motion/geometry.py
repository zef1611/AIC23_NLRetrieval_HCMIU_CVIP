...
def cosine_similarity(vect_a, vect_b):
    if np.isnan(np.dot(vect_a, vect_b)) or np.isinf(np.dot(vect_a, vect_b)):
        return 0
    if np.linalg.norm(vect_a) == 0 or np.linalg.norm(vect_b) == 0:
        return 0
...
# print("test: ", algebra_area(Point(6, 6), Point(6, 0), Point(0, 6)))